...
void loop(){
  ...
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  Serial.println(F("ระบบกำลังทำงาน"));
  Serial.println(F("==================================================="));
  delay(1000);
  Serial.println(F("ระบบกำลังตรวจสอบการทำงาน"));
  delay(1000);
  ...
  if(TWBR == 12 && clockspeed_ok){
    Serial.println(F("กำลังเซ็ตการตั้งค่านาฬิกา 400kHz."));
  }
  else{
    Serial.println(F("ไม่พบการตั้งค่านาฬิกา 400kHz. (ERROR 8)"));
    error = 1;
  }
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("โปรดตรวจสอบ RC Controller ของคุณให้พร้อม"));
    Serial.println(F("==================================================="));
    delay(1000);
    Serial.print(F("กำลังตรวจสอบสัญญาณของ Receiver."));
  ...
  if(error == 0){
    delay(2000);
    Serial.println(F("กรุณาให้ Stick อยู่กึ่งกลางก่อนเวลาจะหมดลง"));
    for(int i = 9;i > 0;i--){
      delay(1000);
      Serial.print(i);
      Serial.print(" ");
    ...
    Serial.println(F(""));
    Serial.println(F("โปรดตรวจสอบ Stick ของคุณว่าอยู่กกึ่งกลางแล้วหรือยัง"));
    Serial.print(F("ค่าสัญญาณอินพุต 08 = "));
    Serial.println(receiver_input_channel_1);
    Serial.print(F("ค่าสัญญาณอินพุต 09 = "));
    Serial.println(receiver_input_channel_2);
    Serial.print(F("ค่าสัญญาณอินพุต 10 = "));
    Serial.println(receiver_input_channel_3);
    Serial.print(F("ค่าสัญญาณอินพุต 11 = "));
    Serial.println(receiver_input_channel_4);
    Serial.println(F(""));
    Serial.println(F(""));
  }
  if(error == 0){  
    Serial.println(F("Stick ด้านซ้าย เร่งคันเร่งและกลับไปตรงกลาง"));
    //Check for throttle movement
    check_receiver_inputs(1);
    Serial.print(F("ขาที่ : "));
    Serial.println((channel_3_assign & 0b00000111) + 7);
    if(channel_3_assign & 0b10000000)Serial.println(F("การเซ็ตค่าของคุณ =ไม่ถูกต้อง"));
    else Serial.println(F("การเซ็ตค่าของคุณ = ถูกต้อง"));
    wait_sticks_zero();
    
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Stick ด้านขวา เลื่อนไปทางด้านขวาและกลับไปตรงกลาง"));
    //Check for throttle movement
    check_receiver_inputs(2);
    Serial.print(F("ขาที่ : "));
    Serial.println((channel_1_assign & 0b00000111) + 7);
    if(channel_1_assign & 0b10000000)Serial.println(F("การเซ็ตค่าของคุณ =ไม่ถูกต้อง"));
    else Serial.println(F("การเซ็ตค่าของคุณ = ถูกต้อง"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Stick ด้านขวา เลื่อนไปด้านล่างและกลับไปตรงกลาง"));
    //Check for throttle movement
    check_receiver_inputs(3);
    Serial.print(F("ขาที่ : "));
    Serial.println((channel_2_assign & 0b00000111) + 7);
    if(channel_2_assign & 0b10000000)Serial.println(F("การเซ็ตค่าของคุณ =ไม่ถูกต้อง"));
    else Serial.println(F("การเซ็ตค่าของคุณ = ถูกต้อง"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Stick ด้านซ้าย เลื่อนไปด้านขวาและกลับไปตรงกลาง"));
    //Check for throttle movement
    check_receiver_inputs(4);
    Serial.print(F("ขาที่ : "));
    Serial.println((channel_4_assign & 0b00000111) + 7);
    if(channel_4_assign & 0b10000000)Serial.println(F("การเซ็ตค่าของคุณ =ไม่ถูกต้อง"));
    else Serial.println(F("การเซ็ตค่าของคุณ = ถูกต้อง"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("ค่อยๆ ขยับ Stick ทั้งหมดไปพร้อมกันขึ้นบน Roll ตามขอบ Stickไปเรื่อยๆ"));
    Serial.println(F("เมื่อพร้อมแล้วให้นำไม้กลับมาที่ตำแหน่งกึ่งกลาง"));
    //Register the min and max values of the receiver channels
    register_min_max();
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("พบค่าสูง , ต่ำ และกลางระหว่างการตั้งค่า"));
    Serial.print(F("Digital input 08 ค่าที่ได้: "));
    Serial.print(low_channel_1);
    Serial.print(F(" - "));
    Serial.print(center_channel_1);
    Serial.print(F(" - "));
    Serial.println(high_channel_1);
    Serial.print(F("Digital input 09 ค่าที่ได้: "));
    Serial.print(low_channel_2);
    Serial.print(F(" - "));
    Serial.print(center_channel_2);
    Serial.print(F(" - "));
    Serial.println(high_channel_2);
    Serial.print(F("Digital input 10 ค่าที่ได้: "));
    Serial.print(low_channel_3);
    Serial.print(F(" - "));
    Serial.print(center_channel_3);
    Serial.print(F(" - "));
    Serial.println(high_channel_3);
    Serial.print(F("Digital input 11 ค่าที่ได้: "));
    Serial.print(low_channel_4);
    Serial.print(F(" - "));
    Serial.print(center_channel_4);
    Serial.print(F(" - "));
    Serial.println(high_channel_4);
    Serial.println(F("Stick ด้านขวา เลื่อนไปทางด้านล่างและกลับไปจุดกลาง"));
    check_to_continue();
  }
  ...
  if(error == 0){
    ...
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("ค้นหา Gyro"));
    Serial.println(F("==================================================="));
    delay(2000);
    
    Serial.println(F("กำลังค้นหาบอร์ด MPU-6050 ที่อยู่ 0x68/104"));
    delay(1000);
    if(search_gyro(0x68, 0x75) == 0x68){
      Serial.println(F("หาที่อยู่ MPU-6050 ไม่พบ 0x68"));
      type = 1;
      gyro_address = 0x68;
    }
    
    if(type == 0){
      Serial.println(F("กำลังค้นหาบอร์ด MPU-6050 ที่อยู่ใหม้อีกครั้ง 0x69/105"));
      delay(1000);
      if(search_gyro(0x69, 0x75) == 0x68){
        Serial.println(F("หาที่อยู่ MPU-6050 ไม่พบ 0x69"));
        type = 1;
        gyro_address = 0x69;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("กำลังค้นหาบอร์ด MPU-6050 ที่อยู่ใหม้อีกครั้ง 0x68/104"));
      delay(1000);
    ...
    if(type == 0){
      Serial.println(F("กำลังค้นหาบอร์ด MPU-6050 ที่อยู่ใหม้อีกครั้ง 0x6A/106"));
      delay(1000);
      if(search_gyro(0x6A, 0x0F) == 0xD7){
        Serial.println(F("หาที่อยู่ MPU-6050 ไม่พบ 0x6A"));
        type = 3;
        gyro_address = 0x6A;
      }
    }
    ...
    if(type == 0){
     Serial.println(F("กำลังค้นหาบอร์ด MPU-6050 ที่อยู่ใหม้อีกครั้ง 0x6B/107"));
      delay(1000);
      if(search_gyro(0x6B, 0x0F) == 0xD7){
        Serial.println(F("หาที่อยู่ MPU-6050 ไม่พบ 0x6B"));
        type = 3;
        gyro_address = 0x6B;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("ตรวจไม่พบอุปกรณ์ Gyro !!! (ERROR 3)"));
      error = 1;
    }
...
void wait_for_receiver(){
  ...
  if(zero == 0){
    error = 1;
    Serial.println(F("."));
    Serial.println(F("ตรวจไม่พบสัญญาณ Receiver!!! (ERROR 1)"));
  }
  else Serial.println(F(" ตรวจพบสัญญาณแล้ว"));
}
...
void register_min_max(){
  ...
  Serial.println(F("การวัดจุดสิ้นสุด...."));
  while(zero < 15){
    if(receiver_input_channel_1 < center_channel_1 + 20 && receiver_input_channel_1 > center_channel_1 - 20)zero |= 0b00000001;
    if(receiver_input_channel_2 < center_channel_2 + 20 && receiver_input_channel_2 > center_channel_2 - 20)zero |= 0b00000010;
    if(receiver_input_channel_3 < center_channel_3 + 20 && receiver_input_channel_3 > center_channel_3 - 20)zero |= 0b00000100;
...
void intro(){
  Serial.println(F("==================================================="));
  delay(500);
  Serial.println(F("Drone Deli✧"));
  delay(500);
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("Website: www.Dronedeli.com"));
}